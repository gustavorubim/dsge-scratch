...
// This file produces the IRFs in the paper, however, these are based on a code that contains some typos, see read me file.
...
//*****************************************************************
beta= 0.9926;       //discount factor NEW
...
kappaw=(1-thetaw*beta)*(1-thetaw)/((1+phi*eta)*thetaw); //NEW
ISS=delta*alpha*(lambdaSS-1)/(lambdaSS*(1/beta-(1-delta))); //NEW steady state value of investment-output ratio
...
//Resource constraint
//*****************************************************************
y=(1-ISS-GSS)*c+ISS*i+GSS*g+alpha*(lambdaSS-1)/lambdaSS*u; //NEW
...
//*****************************************************************
    @#if i==1
        ...
        thetaw=0.75;       //wage Calvo (not CHANGED!!!; in paper thetaw=0.75)
        rhoa=0.9;          //NEU technology persistence SAME AS FOR rhor (no technology shock in CEE)
        rhog=0.9;          //NEU government spending persistence SAME AS FOR rhor (no government spending shock in CEE)
gammab=omegap/(1+beta*omegap);  //backward looking component
gammaf=beta/(1+beta*omegap);    //forward looking component
kappap=(1-thetap*beta)*(1-thetap)/((1+beta*omegap)*thetap);  
kappaw=(1-thetaw*beta)*(1-thetaw)/((1+phi*eta)*thetaw); //NEW
...
//*****************************************************************
    @#if i==2
       ...
       thetap=0.0000001;     //NEW Calvo prob. of not being able to reset the price
       //lambda= depends on the markup shock and lambdaSS, time-varying elasticity of subst. between diff. types of goods
       lambdaSS=6;        // steady staate elasticity of substitution KEPT
       alpha=0.36;        //capital share of output (as in CEE)
       gamma=1;           //degree of cost channel
...
//*****************************************************************
    @#if i==3
       ...
       gammay=0.1;        //output reaction
       GSS=0.2;           //steady state value of the government-output ratio KEPT
       thetaw=0.0000001;     //NEW wage Calvo
...
//figure 4: Baseline estimation
    @#if i==5  
beta= 0.9926;       //NEW discount factor
omegap=0.99;      //price indexation
thetap=0.83;      //Calvo prob. of not being able to reset the price
//lambda= depends on the markup shock and lambdaSS, time-varying elasticity of subst. between diff. types of goods
lambdaSS=6;       // steady staate elasticity of substitution
