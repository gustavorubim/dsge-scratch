...
var stn lnn wrn piyfd pipcd itr itd pieen pimtd pcr lsr xtr mtr ltn pixtd
    ...
    inflationqlag1 inflationqlag2 inflationqlag3
...
interest = const + cofintintb1*interest(-1) + cofintintb2*interestlag1(-1)  //*
	   ...
	   + cofintinf0*inflationq + cofintinfb1*inflationq(-1) 	    //*
	   + cofintinfb2*inflationqlag1(-1) + cofintinfb3*inflationqlag2(-1)//* 
	   + cofintinfb4*inflationqlag3(-1) + cofintinff1*inflationq(+1)    //*
	   + cofintinff2*inflationq(+2) + cofintinff3*inflationq(+3) 	    //*
	   + cofintinff4*inflationq(+4) + cofintout*output 		    //*
...
inflationqlag1 = inflationq(-1);					    //*
inflationqlag2 = inflationqlag1(-1);					    //*
inflationqlag3 = inflationqlag2(-1);					    //*
outputlag1     = output(-1);						    //*
...
shocks;
var interest_          =  1; // because of IRF! 0.1104      ;
