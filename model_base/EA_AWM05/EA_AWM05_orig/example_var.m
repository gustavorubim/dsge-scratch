function [sdpinf, sdygap, sdr, sddr, gamma0] = example_var(scof, cofb, endog_, Omega)
...
Sigma_e(end-1:end,end-1:end)    = Omega;
diff    = 999;
beta0   = A;
...
while diff>10e-6
    ...
    sdpinf  =   sqrt(gamma0(loc(endog_,'pipcd'),loc(endog_,'pipcd')))*400; % inflation rate in annual terms
    sdygap  =   sqrt(gamma0(loc(endog_,'yer'),loc(endog_,'yer')))*100; 
    sdr     =   sqrt(gamma0(loc(endog_,'stn'),loc(endog_,'stn')));
    sddr    =   sqrt(gamma0(loc(endog_,'dstn'), loc(endog_,'dstn')));
    diff    = max([abs(sddr-sddr0), abs(sdpinf-sdpinf0), abs(sdr-sdr0),abs(sdygap-sdygap0)]);
    sdygap0 = sdygap;
    sdpinf0 = sdpinf;
    sdr0    = sdr;
    sddr0   = sddr;
...
sdpinf  =   sqrt(gamma0(loc(endog_,'pipcd'),loc(endog_,'pipcd'))); % inflation rate in annual terms
sdygap  =   sqrt(gamma0(loc(endog_,'yer'),loc(endog_,'yer'))); 
sdr     =   sqrt(gamma0(loc(endog_,'stn'),loc(endog_,'stn')));
sddr    =   sqrt(gamma0(loc(endog_,'dstn'), loc(endog_,'dstn')));
