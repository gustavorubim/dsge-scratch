...
iotap= 0.31;     // price indexation (=0 is static indexation, =1 is dynamic)
iotaw= 0.18;     // wages indexation
gamma100= 0.51;  // steady state growth rate of technology (multiplied by 100)
h= 0.87;         // habit formation 
lambdapss= 0.18; // steady state net price markup
lambdawss= 0.17; // steady state net wage markup
Lss= exp(0.34);       // steady state for log hours
pss100= 1.0064;    // steady state quarterly net inflation (multiplied by 100)
Fbeta= 0.11;     // 100 * ( inv(discount factor) - 1 )
gss = 1/(1-0.22);       // steady state government spending to GDP ratio
niu= 2.43;       // inverse Frisch elasticity
xip= 0.72;       // price stickiness
xiw= 0.50;       // wage stickiness
chi= 5.62;       // elasticity of the capital utilization cost function
Sadj= 2.96;      // investment adjustment cost
fp= 1.94;        // reaction to inflation
fy= 0.06;        // reaction to output gap
fdy= 0.18;       // reaction to output gap growth
rhoR= 0.77;
rhoz= 0.38;
rhog= 0.99;
rhomiu= 0.26;
...
cnustar = 0;    //version for potential output - WHY DO THEY DO THIS???
...
ctheta = 0.95;
cchi = (ctheta - 1)/ctheta ;
//rhoefp = 0.94;   //AR(1) efp shock
//rhonw = 0.79;    //AR(1) nw shock
cb = 0;
*/
