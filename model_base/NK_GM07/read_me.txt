...
+ replicated IRFs: FIg4_original.pdf is fig 4 of the article. Original version with error    
                   FIg4_corrected.pdf is Fig 4 of the article, once the error in the article is addressed    

+ file to produce replicated IRFs: run_original.m  (which calls NK_GM07_error.mod in the folder NK_GM07_rep) 
...
- The article features the collateralized EFP (CEFP) and the uncollateralized EFP (UEFP). In terms of log-deviations from 
    ...
    and CEFP_ss and UEFP_ss are the steady state values.
    
    In both codes, the erroneous and the correct one, the interest rates rT, rIB, rL and rB are in absolute deviations from their 
